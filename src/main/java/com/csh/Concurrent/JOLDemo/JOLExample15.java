...
public class JOLExample15 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        t1.start();
        t1.join();
        out.println("t1添加对象完成:此时list中所有对象为偏向锁，偏向线程t1");
        //偏向
        ...
        Thread t2 = new Thread() {
            ...
            public void run() {
                ...
                for (int i = 0; i <50 ; i++) {
                    ...
                    synchronized (a){
                        if (k==18||k==19){
                            ...
                            //此时实例化对象
                            A aaa = new A();
                            System.out.println("撤销偏量锁达到："+(k+1)+"次 实例化新对象结果");
                            out.println(ClassLayout.parseInstance(aaa).toPrintable());
                        }
                ...
                //此时实例化对象
                A aaa = new A();
                System.out.println("撤销偏量锁达到："+ k +"次 实例化新对象结果");
                out.println(ClassLayout.parseInstance(aaa).toPrintable());
